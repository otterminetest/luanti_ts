...
export function unmarshal(buf: Uint8Array) {
    // Protocol(4) + PeerID(2) + Channel(1) + Type(1) = 8 bytes minimum header
    if (buf.byteLength < 8) {
        throw new Error(`invalid packet length: ${buf.byteLength}`);
    }
    ...
    switch (p.packetType) {
        ...
        case PacketType.Reliable:
            // Reliable header is 3 bytes (SeqNr:2, SubType:1) -> 8 + 3 = 11 bytes min
            if (buf.byteLength < 11) {
                throw new Error("packet too short for Reliable");
            }
            ...
            switch (p.subType) {
                case PacketType.Control:
                    if (buf.byteLength < 13) {
                        // Control in reliable needs Type(1) + PeerID(2) = 3 extra bytes -> 14 total usually,
                        // but let's check basic bounds before reading
                        if (buf.byteLength < 12)
                            throw new Error("packet too short for Reliable Control");
                    }
                    p.controlType = dv.getUint8(11);
                    if (p.controlType === ControlType.SetPeerID) {
                        if (buf.byteLength < 14) throw new Error("packet too short for SetPeerID");
            ...
            break;
        case PacketType.Control:
            // Control header is 8 bytes + Type(1) + SeqNr(2) = 11 bytes min
            if (buf.byteLength < 11) {
                throw new Error("packet too short for Control");
            }
    ...
    throw new Error(
        `not implemented yet: packetType: ${p.packetType}, subtype: ${p.subType}, controlType: ${p.controlType}, data: ${dumpPacket(buf)}`,
    );
}
